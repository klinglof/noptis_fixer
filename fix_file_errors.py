...
def apply_fixes(schema_root_path: pathlib.Path, fix_path: pathlib.Path):
    ...
    for file_path in schema_file_paths:
        ...
        if file_path.name in 'ROI-FromPubTrans.xsd':
            fixed_content = fix_incorrect_schema_version(fixed_content)
...
def generate_library(fixed_files_path,
    ...
    # Run twice because: StopScope, ScopeElements, ScopeElement,
    # SubscriptionMessages and JourneyScope are duplicate types in schema
    # and only last defined will be kept (Preferably FromPubTrans).
