...
def apply_fixes(schema_root_path, fix_path):
...
def find_schema_files(path):
    schema_files = [
        name for name in path.iterdir() if str(name).endswith('.xsd')
    ]
    schema_files.extend([
        name for name in pathlib.Path(path, 'PTShared').iterdir()
        if str(name).endswith('.xsd')
    ])
    return schema_files
...
def generate_library(fixed_files_path, package_name='autogenerated.noptis'):
    ...
    os.system(f'yapf -ir {package_name.replace(".", os.sep)}')


if __name__ == '__main__':
    SCHEMA_PATH = pathlib.Path('test/raw')
    FIX_PATH = pathlib.Path('autogenerated', 'fixed')

    apply_fixes(SCHEMA_PATH, FIX_PATH)

    generate_library(FIX_PATH)
